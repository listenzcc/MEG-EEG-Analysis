...
filenames = [e for e in Path('data').iterdir()
             ...
             and e.name.startswith(subject_name_table['S01'])]
filenames
...
# %%
X, Y, G, ch_names = _get_x_y_g(filenames)
...
X = _select_mode(X, ch_names, mode='EEG')
...
def train_split(dataset, G=G, valid_group=valid_group):
    X = dataset.X
    Y = dataset.y
    train = skorch.dataset.Dataset(X[G != valid_group], Y[G != valid_group])
    valid = skorch.dataset.Dataset(X[G == valid_group], Y[G == valid_group])
